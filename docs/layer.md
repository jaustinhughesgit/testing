...
- a reset-enveloped two-user Compute-sharing scenario that builds with one profile, discovers the exact shared manifest with another profile's 20/200 ordinary evidence, requires app-scoped entity-use IDs in the new local Path, invokes through the deployed worker, and verifies only that user's ContextDB changed;
...
The cross-user Compute-sharing runner requires two existing profiles and an explicit reset confirmation. The CLI performs the canonical reset before the first input and again in a final cleanup path, including after scenario failure. User 1 executes ordinary published Essence setup, authors the hard-stop Convert capability, invokes it, and queries the updated local graph. User 2 starts with a separate graph and no creator Path, executes ordinary setup, submits the invocation plus up to 20 ordinary inputs and 200 ordinary graph entities, and requires Position discovery to return User 1's exact Compute entity/version. The returned model proposal must contain one exact dependency-to-property/subject/relation binding per manifest dependency; the deployed runtime applies that binding and a final published Essence query proves User 2's relation changed. The harness records `copiedCreatorPath: false` and `generatedLocalPath: true`; browser persistence and worker acknowledgement still require the existing website integration tests.

Because canonical reset deletes accounts as well as entities and Paths, the reset-enveloped sharing command clears only the explicitly named test profiles and bootstraps two fresh public users before its first scenario input. Reusing a pre-reset workspace or token is not accepted as a clean start; the final canonical reset still runs after either success or failure.
